...
10. After pushing & opening the PR — close the review loop (see "After pushing"):
    - Run `/code-review max --fix` yourself (via the `Skill` tool), apply its fixes, and re-run npm run check
...
At the end of a dev cycle (changes just pushed), run `/code-review max --fix` — a deep, multi-agent review that fans finder/verifier/sweep agents over the branch diff and applies its findings to the working tree.

- **Run it yourself via the `Skill` tool.** `max` is the highest effort level Claude can launch directly (`low`…`max`, optionally `--fix` / `--comment`), so the review loop no longer stalls waiting on the user.
- It reviews the current working diff; pass a PR number (`/code-review max <PR#> --fix`) to review a specific GitHub PR instead. `--fix` applies the review's findings to the working tree after it completes (use `--comment` instead to post them as inline PR comments).
...
- For an even deeper pass, `/code-review ultra --fix` runs the review in the cloud — but it is **user-triggered and billed, and Claude cannot launch it** (not via Bash, the `Skill` tool, or otherwise). Prompt the user if you want it; don't attempt to invoke it yourself.
