...
describe('getLocalDataInventory (disk-only fallback)', () => {
  ...
  it('computes per-period sizes from disk instead of zero (issue: months showed 0B)', async () => {
    ...
    const inv = await getLocalDataInventory(tempDir, provider, 'daily');
  ...
  it('isolates tiers — hourly raw is not counted under daily', async () => {
    ...
    const daily = await getLocalDataInventory(tempDir, provider, 'daily');
    expect(daily.local.diskBytes).toBe(100);
    expect(daily.periods.find(p => p.period === '2026-06')?.totalSize).toBe(100);

    const hourly = await getLocalDataInventory(tempDir, provider, 'hourly');
...
describe('hasSyncedTier', () => {
  ...
  it('is false when no etag file exists (e.g. imported snapshot)', async () => {
    expect(await hasSyncedTier(tempDir, provider, 'daily')).toBe(false);
  });

  it('is true once the tier etag file exists, per tier', async () => {
    const metaDir = join(tempDir, 'aws', 'meta');
    await mkdir(metaDir, { recursive: true });
    await writeFile(join(metaDir, 'sync-etags.json'), '{}');
    expect(await hasSyncedTier(tempDir, provider, 'daily')).toBe(true);
    // hourly uses its own etag file → still not synced
    expect(await hasSyncedTier(tempDir, provider, 'hourly')).toBe(false);

    await writeFile(join(metaDir, 'sync-etags-hourly.json'), '{}');
    expect(await hasSyncedTier(tempDir, provider, 'hourly')).toBe(true);
  });
});
